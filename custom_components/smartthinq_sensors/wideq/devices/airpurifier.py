"""Support for air purifier devices."""
...
type StateKey = str | list[str]
type CommandKeys = list[StateKey]
type FilterFeatureSet = tuple[
    AirPurifierFeatures, AirPurifierFeatures, AirPurifierFeatures
]
type FilterDef = tuple[
    FilterFeatureSet,
    list[str],
    list[str],
    list[str] | None,
...
class AirPurifierDevice(Device):
    ...
    def __init__(
        ...
        """Initialize the air purifier device."""
        super().__init__(
            client,
            device_info,
            AirPurifierStatus(self),
    ...
    async def set_fan_preset(self, preset: str) -> None:
    ...
    def reset_status(self) -> AirPurifierStatus:
...
class AirPurifierStatus(DeviceStatus):
    ...
    def __init__(
        self, device: AirPurifierDevice, data: dict[str, Any] | None = None
    ) -> None:
        """Initialize device status."""
        super().__init__(device, data)
        self._operation: str | None = None

    def _get_operation(self) -> AirPurifierOp | None:
    ...
    @property
    def current_humidity(self) -> str | None:
        ...
        if (value := self.to_int_or_none(self.lookup_range(key))) is None:
            return None
        return self._update_feature(AirPurifierFeatures.HUMIDITY, str(value), False)
    ...
    @property
    def filters_life(self) -> dict[AirPurifierFeatures, int]:
        """Return percentage status for all filters."""
        result: dict[AirPurifierFeatures, int] = {}
